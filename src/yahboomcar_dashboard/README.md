...
- `/cmd_vel` 控制指令
- 激光雷达点数 / 最近 / 最远距离
- **虚拟摇杆**：两个圆盘式摇杆（左：vx/vy 麦轮平移；右：ωz 旋转），
  鼠标 / 触屏拖动，松开自动归零，可调最大速度，急停按钮
- **2D 地图**：在 Canvas 上以 odom 坐标系实时绘制
  - 1m / 0.5m 网格 + 坐标轴 + 比例尺
  - 蓝色：里程计轨迹（按 2cm 距离过滤，最多 2000 点）
  - 红色：激光点云（自动用当前 yaw 投影到 odom 系）
  - 绿色三角：机器人当前位姿
  - 支持滚轮缩放、关闭跟随后可鼠标拖动平移、清空轨迹
...
| 话题 | 类型 | 来源 |
|---|---|---|
| `cmd_vel` | `geometry_msgs/Twist` | 网页虚拟摇杆 / 急停按钮 |

> 摇杆按下时浏览器 20 Hz 心跳；若 400 ms 无心跳节点会主动发布零速并停止发布，
> 让键盘 / 手柄等其他控制器可以无冲突接管。
...
- `POST /api/cmd_vel` — `{vx, vy, wz}`，发布一次到 `/cmd_vel`
  - 服务端硬限幅：vx/vy ∈ [-1, 1] m/s，wz ∈ [-3, 3] rad/s
