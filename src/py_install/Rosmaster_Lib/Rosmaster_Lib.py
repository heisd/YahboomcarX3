...
class Rosmaster(object):
    ...
    def __init__(self, car_type=1, com="/dev/myserial", delay=.002, debug=False):
        ...
        self.CARTYPE_MECANUM_MINI = 0x01
        self.CARTYPE_MECANUM_MAX = 0x02
        self.CARTYPE_FOURWHEEL = 0x04
        self.CARTYPE_ACKERMAN = 0x05
    ...
    def __parse_data(self, ext_type, ext_data):
        ...
        # the original gyroscope, accelerometer, magnetometer data
            ...
            self.__mx = struct.unpack('h', bytearray(ext_data[12:14]))[0] / 1000.0
            self.__my = struct.unpack('h', bytearray(ext_data[14:16]))[0] / 1000.0
            self.__mz = struct.unpack('h', bytearray(ext_data[16:18]))[0] / 1000.0
        # 解析板子的姿态角
    ...
    def set_motor(self, speed_1, speed_2, speed_3, speed_4, pid=True):
        try:
    ...
    def set_car_run(self, state, speed, adjust=False):
        try:
            ...
            t_speed = bytearray(struct.pack('h', self.__limit_motor_value(speed)))
    ...
    def set_car_type(self, car_type):
        if str(car_type).isdigit():
            self.__CAR_TYPE = int(car_type)
            if self.__debug:
                print("Set CAR_TYPE:", self.__CAR_TYPE)
        else:
...
if __name__ == '__main__':
    ...
    # bot.set_car_type(bot.CARTYPE_ACKERMAN)
